...
def test_comm(drugs):
    ...
    with model:
        ...
        nengo.Connection(A, B_LIF, synapse=tau, transform=tau)
        nengo.Connection(A, B_nrn, solver=solver, synapse=ExpSyn(k_synapse*tau), transform=tau)
    ...
    sim.run(t_stim+t_delay)
    return sim,stim_probe,A_probe,B_LIF_probe,B_nrn_probe,C_LIF_probe,C_nrn_probe

drugs=[['control']]
sim,stim_probe,A_probe,B_LIF_probe,B_nrn_probe,C_LIF_probe,C_nrn_probe=test_comm(drugs)
...
ax1.legend()
ax2.plot(sim.data[A_probe],label='sensory')
ax2.set_xlabel("t")
ax2.legend()
ax3.plot(sim.data[B_LIF_probe],label='wm_LIF')
