...
drugs_effect={'control':0.00,'PHE':-0.01,'GFC':0.01} #for additive_noise, [gfc_mean, phe_mean];
ramp_scale=0.5 #how fast does the 'time' dimension accumulate in WM neurons
...
with model:
	...
	#Probes
	probe_wm=nengo.Probe(wm[0],synapse=0.1,sample_every=dt_sample) #no ramp information passed
    ...
    for n in trials:
		...
		for t in timesteps:
			...
			if (cues[n] > 0.0 and output > 0.0) or (cues[n] < 0.0 and output < 0.0):
				...
				correct=0
			rt=t*dt_sample
			dataframe.loc[i]=[rt,drug,wm,output,correct,n]
			i+=1
...
ax1.set(xlabel='',ylabel='|WM value|')
