...
frac=0.005 #fraction of neurons in WM to add to dataframe and plot

neurons_sensory=200 #neurons for the sensory ensemble
neurons_wm=100 #neurons for workimg memory ensemble
neurons_decide=500 #neurons for decision or basal ganglia
...
def firing_dataframe(sim,drug,trial,probe_spikes):
...
def my_simulator(params):
	...
	with nengo.Simulator(model,dt=dt) as sim:
		...
		df_firing=firing_dataframe(sim,drug,trial,probe_spikes)
...
# 	for trial in trials:
# 		queue=Queue()
# 		proc=Process(target=my_simulator, args=(drug,trial,queue))
# 		proc.start()
# 		queues.append(queue)
# 		processes.append(proc)

# for i in range(len(processes)):
# 	print 'primary dataframe', queues[i].get()[0]
# 	print 'firing dataframe', queues[i].get()[1]
# 	processes[i].join()
